...
def plot(img_index, model_outputs, ground_truth, path, epoch, img_res, is_eval=False, first=False, save_compare=True):
    # arrange data to plot
    batch_size = model_outputs['batch_size']
    return plot_images(model_outputs, ground_truth, path, epoch, img_index, 1, img_res, batch_size, is_eval, save_compare=save_compare)
...
def plot_images(model_outputs, ground_truth, path, epoch, img_index, plot_nrow, img_res, batch_size, is_eval, log_err=True, save_compare=True):
    ...
    if log_err and 'rgb' in ground_truth:
        gt = ground_truth['rgb']
        pred = model_outputs['rgb_image'].reshape(batch_size, num_samples, 3)
        # mask = ground_truth['object_mask'].reshape(batch_size, num_samples, 1)
        error = (gt - pred.clamp(0, 1))**2

        output_vs_gt = torch.cat((output_vs_gt, error), dim=0)


    extra_plot_keys = ['layer_bg', 'gaussian_only', 'layer_coarse', 'layer_fine', 'render_img_gs']

    for k in extra_plot_keys:
        if k in model_outputs:
            if k == 'render_img_gs':
                model_outputs[k] =  model_outputs[k].reshape([1, 3, -1]).permute([0,2,1])
            output_vs_gt = torch.cat((output_vs_gt, model_outputs[k]), dim=0)
    ...
    tensor = tensor.transpose(1, 2, 0)
    scale_factor = 255
    tensor = (np.clip(tensor, 0., 1.) * scale_factor).astype(np.uint8)
    img = Image.fromarray(tensor)
    wo_epoch_path = path[0].replace('/epoch_{}'.format(epoch), '')

    Path('{0}/rendering'.format(wo_epoch_path)).mkdir(exist_ok=True, parents=True)
